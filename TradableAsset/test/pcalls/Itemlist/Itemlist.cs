namespace Expload.Standards {
    ...
    public class Itemlist {
        ...
        public void test_Itemlist() {
            ...
            // Get asset counter
            long assetCounter = ProgramHelper.Program<TradableGTAsset>(programOwner).GetUsersGTAssetCount(assetOwner);
            ...
            long actualAssetId1 = ProgramHelper.Program<TradableGTAsset>(programOwner).GetUsersGTAssetId(assetOwner, 0);
            ...
            long actualAssetId2 = ProgramHelper.Program<TradableGTAsset>(programOwner).GetUsersGTAssetId(assetOwner, 1);
            ...
            ProgramHelper.Program<TradableGTAsset>(programOwner).TransferGTAsset(assetId1, newAssetOwner);

            // Check if asset was actually taken
            assetCounter = ProgramHelper.Program<TradableGTAsset>(programOwner).GetUsersGTAssetCount(assetOwner);
            if(assetCounter != 1) {
                Error.Throw("Transfer went wrong! (wrong assets amount)");
            }

            // Check if last asset was moved to the first slot
            long movedAssetId = ProgramHelper.Program<TradableGTAsset>(programOwner).GetUsersGTAssetId(assetOwner, 0);
